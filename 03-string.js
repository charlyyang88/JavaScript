...
//  ES는 문자열도 배열처럼 활용
console.log(s1[7]);
console.log(s1.substr(7,10));       //  부분문자열: 7번 인덱스부터 10개 문자 추출(공백제외) -> deprecated
...
console.log("1st serarch:", position);
position += 4;
...
//  변경 데이터를 뒤에서도 사용 하고자 한다면, 재할당 후 사용
const s1_changed = s1.replace("JavaSctipt", "JS");  //  s1의 JavaScript를 JS로 치환한 결과를 재할당
console.log(s1, s1_changed);    //  원본 데이터가 그대로 남아있음
